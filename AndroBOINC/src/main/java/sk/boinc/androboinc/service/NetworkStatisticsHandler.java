...
public class NetworkStatisticsHandler implements NetStats, OnSharedPreferenceChangeListener {
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
		if (key.equals(PreferenceName.COLLECT_STATS)) {
			...
			// Store new value
			if (BuildConfig.DEBUG) Log.d(TAG, "Collection flag changed from " + mCollection + " to " + collection);
	...
	public void connectionOpened() {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (BuildConfig.DEBUG) Log.d(TAG, "New connection opened");
	...
	public void bytesReceived(final int numBytes) {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (BuildConfig.DEBUG) Log.d(TAG, "Received " + numBytes + " bytes, uncommitted: " + mUncommittedReceived + " bytes");
				checkpoint();
			}
		});
	}
	...
	public void bytesTransferred(final int numBytes) {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (BuildConfig.DEBUG) Log.d(TAG, "Sent " + numBytes + " bytes, uncommitted: " + mUncommittedSent + " bytes");
				checkpoint();
			}
		});
	}
	...
	public void connectionClosed() {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				commitPending();
				if (BuildConfig.DEBUG) Log.d(TAG, "Connection closed, total received: " + mTotalReceived + " bytes, sent: " + mTotalSent + " bytes");
			}
		});
	}
	...
	private void checkpoint() {
		if ((mUncommittedReceived + mUncommittedSent) > CHKPNT_THRES_SIZE) {
			...
			if (BuildConfig.DEBUG) Log.d(TAG, "Uncommitted total: " + (mUncommittedReceived + mUncommittedSent) + ", checkpoint");
			commitPending();
		}
	}
